...
.relabel_samples <- function(object, relabels) {
    Subject_ID <- Sample_ID <- Genotype_Group_ID <- Sample_ID.y <- 
        Subject_ID.y <- Subject_ID.x <- Is_Ghost <- NULL
    ...
    for (i in seq_len(nrow(mislabeled_genotype_swapcats))) {
        ...
        if (allow_ghosts) {
            ...
            ## If we have more ghost labels than needed, select a subset
                ...
                ghost_labels <- ghost_labels[seq_len(n_ghost_labels)]
            }
        }
        ...
        if (allow_unknowns) {
            ...
            if (n_unknown_labels > 0) {
                unknown_labels <- seq_len(n_unknown_labels)
