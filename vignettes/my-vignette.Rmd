...
# symmetric boolean matrix, where 
...
Alternatively, the genotype group information can be provided as a square, 
symmetric boolean genotype_matrix, with rownames and column names coming from 
the Sample_ID column in the sample_metadata and a TRUE indicates that two samples 
...
should match the equivalent column in sample_metadata, and another for SwapCat_ID 
which groups the Sample_ID(s) which may be swapped for one another. For example,
if dataset involves various assay modalities, the modality of each sample
would be a swappable category provided that where sample swaps can only occur 
between samples of the same modality. While this input is optional, the 
additional information provided helps guide and constrain the solver.
...
and provide that information to the solver through anchor_samples. This is 
...
FOMO provides three solver functions to find likely relabels: solveMajority, 
solveComprehensiveSearch, solveLocalSearch, and solveEnsemble. The recommended 
solver function is solveEnsemble.
...
writeOutput(mislabel_solver_solved, "~/outputted_corrections")
